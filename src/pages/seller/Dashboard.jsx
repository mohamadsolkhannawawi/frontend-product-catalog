                    ...
                    {(stats?.latest_products || []).length > 0 ? (
                      (stats?.latest_products || []).map((p) => {
                        ...
                        const productsArray = Array.isArray(products)
                          ? products
                          : products?.data || [];
                        const fullProduct = productsArray.find(
                          (item) => item.product_id === p.product_id
                        );

                        // Gunakan data dari fullProduct jika ada, jika tidak gunakan p, fallback ke 0
                        const displayStock = fullProduct?.stock ?? p.stock ?? 0;
                        const displayPrice = fullProduct?.price ?? p.price ?? 0;
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-xs font-medium text-purple-600 bg-purple-50 px-2 py-1 rounded">
                              Rp{" "}
                              {parseInt(displayPrice).toLocaleString("id-ID")}
                            </span>
                          </div>
                        );
                      })
