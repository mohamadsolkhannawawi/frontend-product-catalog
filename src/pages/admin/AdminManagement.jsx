        ...
        <div className="space-y-6 pb-10">
            ...
            <div className="flex justify-between items-start">
                <div className="text-left">
                    <h1 className="text-2xl font-bold text-gray-800">Manajemen Penjual</h1>
                ...
                <div className="flex-1 overflow-x-auto">
                    {loading ? (
                        <div className="h-64 flex items-center justify-center text-gray-500 gap-2">
                            <Loader className="animate-spin w-5 h-5" /> Memuat data...
                        </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-100">
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Nama Toko</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Provinsi</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">No. Telepon</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Upload Terakhir</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Status</th>
                                    <th className="px-6 py-4 text-[11px] font-bold text-gray-400 uppercase tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {paginatedData.map((seller) => (
                                    <tr key={seller.seller_id} className="hover:bg-gray-50/50 transition-colors group">
                                        <td className="px-6 py-4 align-middle">
                                            <div>
                                                <div className="text-sm font-bold text-gray-800">{seller.store_name}</div>
                                                <div className="text-xs text-gray-400 mt-0.5">{seller.user?.email || "No Email"}</div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 align-middle">
                                            <button
                                                onClick={() => toggleSellerStatus(seller.seller_id, seller.is_active)}
                ...
                {filteredData.length > 0 && (
                    <div className="border-t border-gray-100 p-4 bg-white flex items-center justify-between mt-auto">
                        <p className="text-xs text-gray-500">
                            Menampilkan <span className="font-bold text-gray-700">{(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredData.length)}</span> dari <span className="font-bold text-gray-700">{filteredData.length}</span> penjual
                        </p>
