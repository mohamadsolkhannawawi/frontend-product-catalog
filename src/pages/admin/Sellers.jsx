...
export default function AdminSellers() {
    ...
    const [selectedPreviews, setSelectedPreviews] = useState({ ktp: null, pic: null });
    ...
    const openDetails = async (sellerId) => {
        ...
        try {
            const res = await api.get(`${API_ENDPOINTS.ADMIN_SELLERS}/${sellerId}`);
            const seller = res.data || res;
            setSelectedSeller(seller);

            const ktpUrl = seller.ktp_file_path ? await fetchBlobUrl(seller.seller_id, "ktp") : null;
            const picUrl = seller.pic_file_path ? await fetchBlobUrl(seller.seller_id, "pic") : null;
                                    ...
                                    <tr key={seller.seller_id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="p-4 align-top">
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-500 line-clamp-1 max-w-[180px]">
                                                    {seller.store_description || "Elektronik & Gadget"}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="p-4 align-top">
                                            <div className="flex items-center gap-3">
                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-100 to-purple-200 flex items-center justify-center text-purple-700 font-bold text-xs">
                                                    {seller.user?.name?.substring(0, 2).toUpperCase() || "PIC"}
                                                </div>
                                                <div>
                                                    <div className="font-semibold text-gray-800 text-sm">{seller.user?.name}</div>
                                        ...
                                        <td className="p-4 align-top">
                                            <div className="flex gap-2">
                                                <button 
                                                    onClick={() => previewFile(seller.seller_id, "ktp")}
                                                ...
                                                <button 
                                                    onClick={() => previewFile(seller.seller_id, "pic")}
                                                    className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-purple-200 text-purple-600 text-xs font-medium rounded hover:bg-purple-50 transition"
                                                >
                                        ...
                                        <td className="p-4 align-top">
                                            <div className="flex items-center justify-center gap-2">
                                                <button 
                                                    onClick={() => openDetails(seller.seller_id)}
                                                ...
                                                <button 
                                                    onClick={() => approve(seller.seller_id)}
                                                ...
                                                <button 
                                                    onClick={() => {
                                                        setRejecting({ open: true, sellerId: seller.seller_id });
                                                        setReason("");
                                                    }}
